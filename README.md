...
| `REVIEW_PROFILES` | | `default` | 多模型交叉 review 的 codex profile 列表（见下文） |
...
allowFix: false           # true = 开启 /fix（bot 可向 PR 源分支推修复提交）
...
配置：模型定义放 codex（`CODEX_HOME/config.toml`），bot 只配 profile 名：
...
- **默认全局关闭**，按仓库 `.ai-review.yml` 设 `allowFix: true` 开启（该文件读自 PR 源分支）；
