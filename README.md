...
| 你做什么 | bot 做什么 |
|---|---|
| 建 PR（或草稿转正式） | 自动全量 review：总评（摘要 + 变更导览 + 风险）+ 行内问题评论 + PR status |
...
**其它特性**：走 Service Hooks 事件推送，不占用 build agent、无轮询；多 PR 并行、同一 PR 串行去重；review 结果可推送 RocketChat / 企业微信；提示词、review 规则全部可配置。
...
| `NOTIFY_EVENTS` | | 全部 | `review_completed,must_fix_found,job_failed` |
...
**提示词模板**（`prompts/` 目录，改动即生效、无需重启）：`review-full.md` 全量 / `review-incremental.md` 增量 / `qa.md` 问答。
